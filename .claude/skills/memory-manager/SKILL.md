...
via el Paso 0 del skill `primer` (no es opcional — en Titan Factory trabajan varios
programadores sobre el mismo repo y la memoria debe viajar con el).

El setup viene pre-instalado de fabrica: `.claude/settings.json` con `"autoMemoryEnabled": false`,
`.claude/memory/MEMORY.md` y las 4 carpetas con `.gitkeep`. Si falta algo (proyecto creado con
una version vieja del template), ejecutar estos pasos de setup:
